        ...
        <Card className="p-5 mt-4">
          ...
          <ul className="space-y-2.5 text-[12.5px] text-ink-600 leading-relaxed">
            ...
            <li>
              ...
              Occupancy and other ratio lines are compared as fractions (an extracted 83 or 83% reads as 0.83; the conversion is noted on the row); currency is compared in whole dollars (a &ldquo;$000&rdquo; unit is scaled). A value whose unit cannot be established — an occupancy above 100, for example — is not compared at all (<ReasonTag code="unit_unknown" />), and the Technical detail says why. A figure whose <em>path</em> declares a unit the concept does not carry is refused under the same code: an OM proforma&apos;s %-of-revenue column (<code className="text-[11.5px]">broker_proforma.rooms_revenue_pct</code>) read against a dollar T-12 line is rejected before any comparison, so it never produces a variance percentage. Fondok does not reconstruct dollars from a percent claim — the denominator would itself be an unverified broker figure.
            </li>
            <li>
            ...
            <li>
              <span className="font-semibold text-ink-900">NOI is compared like-for-like across the FF&amp;E reserve.</span>{' '}
              A broker proforma states NOI <em>before</em> the FF&amp;E replacement reserve (USALI &ldquo;EBITDA&rdquo;), while the T-12 line Fondok normalises to is <em>after</em> it (USALI &ldquo;EBITDA Less Replacement Reserve&rdquo;). Comparing the two reports the reserve as if it were a broker overstatement. So when the actuals document states its own before-reserve line, the broker&apos;s NOI claim is compared against that one, and the row says which figure it used; when the document states only the after-reserve line, the pair is refused as &ldquo;Basis mismatch — needs review&rdquo; (<ReasonTag code="basis_mismatch" />) with both figures and no severity. The before-reserve figure is only ever read from the document — never reconstructed by adding the reserve back.
            </li>
          </ul>
        </Card>
      </Section>
